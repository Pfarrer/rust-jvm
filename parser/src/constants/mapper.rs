...
use model::class::{ClassConstants, ClassConstant};
...
pub fn map(raw_constants: Vec<RawConstant>) -> Result<ClassConstants> {
    let constants = raw_constants.iter().enumerate().map(|(i, raw_constant)| {
        match raw_constant {
            ...
            &RawConstant::Fieldref(class_index, name_and_type_index) => {
                ...
                Ok(ClassConstant::Fieldref(class_name, field_name, type_signature))
            },
            &RawConstant::Methodref(class_index, name_and_type_index) => {
                ...
                Ok(ClassConstant::Methodref(class_name, method_name, method_signature))
            },
            &RawConstant::InterfaceMethodref(class_index, name_and_type_index) => {
                ...
                Ok(ClassConstant::InterfaceMethodref(class_name, method_name, method_signature))
            },
...
fn unwrap_class(raw_constants: &Vec<RawConstant>, class_index: u16) -> Result<String> {
    match raw_constants.get(class_index as usize).context("Raw constant not found")? {
...
fn unwrap_name_and_type(raw_constants: &Vec<RawConstant>, index: u16) -> Result<(String, String)> {
    match raw_constants.get(index as usize).context("Raw constant not found")? {
